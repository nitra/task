...
LLM-агент (`runAgent`) підключено як четвертий адаптер поверх того самого `dispatch`/каталогу — поряд із UI (`invoke`), CLI (`bin/task.mjs`) і MCP (`task mcp`). Наживо перевірено: `gemma-4-e2b-it-4bit` через omlx обрала тул `create` зі спільного каталогу, `dispatch` виконав — задача реально створена на диску. Невалідний тул повертає `{ok:false,error:{code:"not_found"}}` без виклику Rust — валідація за JSON-схемою відбувається до транспорту.

Правило `tool-surface` опубліковано в `@nitra/cursor` як пакетне правило (`npm/rules/tool-surface/tool-surface.mdc`) з авто-активацією на фронтенд-залежностях (`vue`, `react`, `@tauri-apps/api`, `@capacitor/core` тощо). `tauri.mdc` отримав per-stack секцію Tool Surface (версія `1.4 → 1.5`). Канон-специфікація: `docs/specs/260614-n-tool-surface.md`.
